...
class searchWizard_class {
  ...
  function getNextFieldAndValue(&$formFields,&$formValues, $type,$parent,$c,$required=TRUE) {
    ...
    switch ($type) {
      ...
      case 'L':
        ...
        $choices = dbScheme_allFieldsChoices($this->dbParsed,$allFields,$path2table);
        // note that multiple constraints all use the same table T
        $ff = new selectField_class($prefix.'Property to limit search by');
        $ff->setChoices($choices);
        if (isset($val)) {
          $dotPos = strpos($val,'.');
          if ($dotPos !== FALSE) {
      ...
      case 'op':
        ...
        $choices = array('eq' => 'equals'.($trunc ? ' (first '.$trunc.' characters)' : ''),'*x*'=>'contains','x*'=>'starts with','*x'=>'ends with','IN'=>'IN (subquery)');
      ...
      case 'R':
        ...
        if ($op == 'eq') {
          ...
          $params = array();
          if (isset($val)) {
            $q .= ' WHERE '.($trunc ? 'LEFT('.$fullField.','.$trunc.')' : $fullField).'=$R';
            $params['R'] = $val;
          } else {
            $T = $formValues[$parent.'T.table'];
      ...
      case 'x':
        if ($c == 0) {
          ...
          $ff->setChoices(array(''=>'No','AND'=>'Yes, combine using AND','OR'=>'Yes, combine using OR (after AND)','OR!'=>'Yes, combine using OR (before AND)'),'');
        }
        break;
      case 'limit':
        ...
        if (!isset($x)) {
          ...
          if (!isset($L)) {
            ...
            if (empty($op)) {
              ...
              $R = $this->scalarPropertiesForm($formFields,$formValues,$submitText,$parent.'R'.$c,0);
              if (empty($R)) {
  ...
  function submitQuery($formValues,$customScript=NULL) {
    ...
    if ($format == 'altkeys') {
      ...
      formatAs_jsonHeaders();
      echo formatAs_prettyJson($ans['tables'][$resultTable]['altKeys']);
      return;
    }
    ...
    if ($format=='post') {
      return $ans;
    } elseif ($format=='json' || $format=='altkeys') {
      formatAs_jsonHeaders();
      echo formatAs_prettyJson($ans);
    } elseif ($format=='dhtml') {
